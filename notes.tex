...
Brooks theorem. If \(G\) is a graph not isomorphic to an odd-cycle or to a
...
must exists two neighbors \(u, w\) of \(v\) which are not adjacent (why?).
...
\begin{center}
  \begin{tikzpicture}[thick, draw]
    ...
    \node[right=of v2] (v3) {3};
    \node[right=of v3] (v4) {4};

    \node[below=of v1] (v5) {2};
    \node[below=of v2] (v6) {1};
    \node[below=of v3] (v7) {3};
    \node[below=of v4] (v8) {4};
...
\begin{center}
  \begin{tikzpicture}
    \node[draw, dashed, minimum size=2cm, regular polygon,
      regular polygon sides=6] (polygon) {};
    \draw[thick] (polygon.corner 1) -- (polygon.corner 2);
    \draw[thick] (polygon.corner 3) -- (polygon.corner 4);
    \draw[thick] (polygon.corner 5) -- (polygon.corner 6);
  \end{tikzpicture}
  \begin{tikzpicture}
    ...
    \draw[dashed] (polygon.corner 5) -- (polygon.corner 6);
  \end{tikzpicture}
\end{center}
...
\begin{theorem}
  ...
  \(\chi(G) \in \left\{w(G) \,\middle|\, w(G) + 1\right\}\).  Note that \(w(G)\)
  is computable in polynomial time for this class of graphs: each \(K_4\)
  extends in a unique way to a maximal clique.
\end{theorem}
